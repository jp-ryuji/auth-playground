...
.PHONY: help up down logs ps test test-signup discovery wait-hydra

help:                ## Show this help
	@awk 'BEGIN {FS = ":.*?## "} /^[a-zA-Z_-]+:.*?## / {printf "  \033[36m%-14s\033[0m %s\n", $$1, $$2}' $(MAKEFILE_LIST)

up:                  ## Start Hydra + Kratos + Postgres + SSUI + Mailslurper
	docker compose up -d
...
test-signup:         ## Run only the SIGNUP-NN suite
